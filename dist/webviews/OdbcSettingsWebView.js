...
exports.OdbcSettingsWebview = void 0;
...
class OdbcSettingsWebview {
    static instance;
    panel;
    outputChannel;
    context;
    CONNECTION_TYPE_KEY = "irisIO.defaultConnectionType";
    ODBC_DRIVER_KEY = "irisIO.odbcDriver";
    constructor(context, outputChannel) {
        this.context = context;
        this.outputChannel = outputChannel;
    }
    static getInstance(context, outputChannel) {
        if (!OdbcSettingsWebview.instance) {
            OdbcSettingsWebview.instance = new OdbcSettingsWebview(context, outputChannel);
        }
    ...
    async show() {
        ...
        this.panel = vscode.window.createWebviewPanel("irisOdbcSettings", "IRIS Connection Settings", vscode.ViewColumn.One, {
            enableScripts: true,
        ...
        setTimeout(() => {
            const selectedDriver = settingsManager_1.SettingsManager.getOdbcDriver(this.context);
            if (selectedDriver) {
                this.log(`[OdbcSettingsWebview] Sending initial driver: ${selectedDriver}`);
    ...
    async handleMessage(message) {
        switch (message.type) {
            ...
            case "downloadDrivers":
                vscode.env.openExternal(vscode.Uri.parse("https://intersystems-community.github.io/iris-driver-distribution/"));
                break;
            case "setOdbcDriver":
    ...
    async setOdbcDriver(driverName) {
        try {
            this.log(`[OdbcSettingsWebview] Setting ODBC driver to: ${driverName}`);
            await this.context.workspaceState.update(this.ODBC_DRIVER_KEY, driverName);
            this.log(`[OdbcSettingsWebview] Successfully saved ODBC driver: ${driverName}`);
            vscode.window.showInformationMessage(`ODBC driver set to: ${driverName}`);
            this.postMessage("odbc-driver-saved", driverName);
        }
        catch (error) {
            this.log(`[OdbcSettingsWebview] Error saving ODBC driver: ${error.message}`);
            vscode.window.showErrorMessage(`Failed to save ODBC driver: ${error.message}`);
    ...
    getBodyContent() {
        const html = pathHelper_1.PathHelper.readWebviewFile(this.context, "webviews", "settings", "settings.html");
        const css = pathHelper_1.PathHelper.readWebviewFile(this.context, "webviews", "settings", "settings.css");
        return `
      <style>${css}</style>
      ${html}
    `;
    }
    getCustomScript(currentConnectionType) {
        return `
    ...
    getHtmlContent(currentConnectionType) {
...
exports.OdbcSettingsWebview = OdbcSettingsWebview;
//# sourceMappingURL=OdbcSettingsWebView.js.map