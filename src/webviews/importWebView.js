 ...
 */
class ImportWebview extends baseWebView_1.BaseWebview {
    ...
    getBodyContent() {
        const html = pathHelper_1.PathHelper.readWebviewFile(this.context, "webviews", "import", "import.html");
        const css = pathHelper_1.PathHelper.readWebviewFile(this.context, "webviews", "import", "import.css");
  ...
  `;
        ...
        switch (message.type) {
            ...
            case "load-schemas":
                await vscode.window.withProgress({
                    ...
                    await this.handleLoadSchemas(message.data?.filter, message.data?.inputId);
                });
                break;
            case "load-tables":
                await vscode.window.withProgress({
                    ...
                    await this.handleLoadTables(message.data.schema, message.data.inputId);
                });
                break;
            case "browse":
     ...
     */
    async handleProgressCommand(data) {
        await vscode.window.withProgress({
            location: vscode.ProgressLocation.Notification,
            title: data.title,
            cancellable: false,
            ...
            switch (data.command) {
                ...
                case "import":
                    ...
                    break;
                default:
                    this.log(`[ImportWebview] Unknown command in run-with-progress: ${data.command}`);
                    break;
            }
        });
    }
    async handleLoadSchemas(filter, inputId) {
        try {
    ...
    async handleLoadTables(schema, inputId) {
        try {
    ...
    async handleAnalyzeFile(data) {
        try {
            this.log(`[ImportWebview] Analyzing file: ${data.filePath}`);
            // Use connector's analyzeFile method
            const analysis = await this.connector.analyzeFile(data.filePath, data.fileFormat, data.delimiter);
            this.log(`[ImportWebview] File analysis complete: ${analysis.columns.length} columns`);
    ...
    async handleImport(data) {
        try {
            ...
            if (data.mode === "create") {
                ...
                await this.connector.importToNewTable(data.filePath, data.tableName, data.schema, data.fileFormat, data.columnTypes, data.columnIndexes, data.delimiter);
            }
            else {
                // Import into existing table
                await this.connector.importToExistingTable(data.filePath, data.tableName, data.schema, data.fileFormat, data.dataAction || "append", data.delimiter);
            }
            vscode.window.showInformationMessage(`Data imported successfully into ${data.schema}.${data.tableName}`);
...
exports.ImportWebview = ImportWebview;
//# sourceMappingURL=importWebView.js.map